...
매칭 성공 시, 월요일 오후 4시에 이 채팅방을 통해 전달해 드릴 예정입니다. 두근거리는 마음으로 조금만 기다려 주세요! 😊`;
...
function meetingTimeWithoutYear(meetingTime) {
  return String(meetingTime ?? '').trim().replace(/^\d{4}년\s*/, '');
}

exports.buildMatchCompleteText = function buildMatchCompleteText(meetingTime, meetingPlace) {
  return exports.MATCH_COMPLETE_INTRO_TEXT.replace(
    '#{미팅일시}',
...
exports.buildMatchDayEveReminderText = function buildMatchDayEveReminderText(params) {
  const meetingTime = meetingTimeWithoutYear(params?.meetingTime);
