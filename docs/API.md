...
`energy`, `weekend`, `pattern`, `trend`, `alcohol`, `smoking`, `tattoo`, `contact`, `meeting`, `planning`, `affection`, `date_expense`, `friends`, `jealousy`, `skinship_speed`, `skinship_limit`, `date_drinking`, `politics`, `religion_type`, `religion_intensity`, `marriage_view`, `meeting_seriousness`, `job_view`, `spending`, `conflict`, `empathy`, `honesty`, `trust`, `pref_cc`, `pref_smoking`, `pref_tattoo`, `pref_religion`

**응답**

| HTTP | 본문 |
|------|------|
| `200` | `{ "message": "설문 결과가 저장되었습니다.", "userId": string }` — `userId`는 `Trait.id`(=`Identity.id`) |
...
| `inputUsers` | array | `{ id, mbti }[]` |
| `comparisons` | array | 각 원소: `user_A`, `user_B` — `{ id, mbti }`, `match` — Python `CalculateMatchResponse` 본문 |
...
인증된 유저(`x-user-uuid` = `Identity.id`)의 `Trait.surveyData`를 기준으로, DB에 설문이 있는 **다른** `Identity`들과 Python `POST /calculate-match`를 **후보마다 순차 호출**한 뒤, **가장 높은 `final_score`** 인 상대 한 명만 반환합니다. 이메일·UUID·상대 id는 내려주지 않습니다.
...
| HTTP | 본문 |
|------|------|
| `200` | `{ "partnerLabel": string, "score": number, "report": object }` — `partnerLabel`은 상대 `Trait.mbti`(비어 있으면 `"상대"`), `report`는 Python `match_report` |
| `400` | `{ "error": "설문을 먼저 제출해 주세요." }` — 본인 `surveyData` 없음 |
| `401` | 세션 헤더 오류 |
| `404` | `{ "error": "매칭할 다른 사용자가 없습니다." }` |
